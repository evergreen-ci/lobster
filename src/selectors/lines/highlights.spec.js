...
test("selectors-shouldHighlightLine", function () {
  ...
  lines.forEach((line) =>
    expect(
      shouldHighlightLine(line, highlightRegexps, highlightLinesRegexp, false)
    ).toBe(true)
  );
  ...
  [lines[0], lines[2], lines[3], lines[4], lines[5]].forEach((line) =>
    expect(
      shouldHighlightLine(line, highlightRegexps, highlightLinesRegexp, false)
    ).toBe(false)
  );
...
test("selectors-getHighlightText", function () {
  ...
  expect(getHighlightText(highlights)).toEqual(
    expect.arrayContaining(["Line 4"])
  );
});
