...
describe("e2e", function () {
  ...
  e2e("highlight", async (done) => {
    ...
    try {
      ...
      expect(await driver.getCurrentUrl()).toBe(
        `http://${lobsterServer()}/lobster/logdrop#bookmarks=0%2C6&h~=100~Line%20&l=1&server=${encodeURIComponent(
          lobsterServer()
        )}%2Fapi%2Flog&url=simple.log`
      );
  ...
  e2e("filter", async (done) => {
    ...
    try {
      ...
      expect(await driver.getCurrentUrl()).toBe(
        `http://${lobsterServer()}/lobster/logdrop#bookmarks=0%2C6&f~=100~Line%20&l=1&server=${encodeURIComponent(
          lobsterServer()
        )}%2Fapi%2Flog&url=simple.log`
      );
      ...
      await l.addFilter();
      expect(await driver.getCurrentUrl()).toBe(
        `http://${lobsterServer()}/lobster/logdrop#bookmarks=0%2C6&f~=101~Line%20&f~=100~2&l=1&server=${encodeURIComponent(
          lobsterServer()
        )}%2Fapi%2Flog&url=simple.log`
      );
      await l.showDetails();
  ...
  e2e("changeurl", async (done) => {
    ...
    try {
      ...
      await l.get(
        `${lobsterURL("simple.log")}#bookmarks=0,6&f=10Line&h=11Line 4`
      );
