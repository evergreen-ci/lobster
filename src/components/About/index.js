...
const About = () => (
  ...
  Lobster is a log viewer implemented as a React-based frontend.  It also includes a node based backend to view local log files or cache the results locally.
    ...
    <p> Lobster can: </p>
    <ul>
      <li>search any bookmarked, filtered, or otherwise visible lines by regexp (the &quot;Find&quot; button). If the regexp finds multiple occurrences in the same log line, it will only count one, but it will highlight them all. As of now, it will also not scroll to the right if the matching text is off screen.</li>
      ...
      <li>double click on a line to bookmark (or unbookmark) that line and click on that line number on the left-hand side to jump to it</li>
