...
export class LogDrop extends React.PureComponent<Props, State> {
  ...
  selectCallback = (select: ?HTMLSelectElement) => {
    this.select = select;
  };
  ...
  render() {
    ...
    if (this.state.error != null) {
      ...
      if (this.state.files.length !== 1) {
        child = (
          <div
            ...
            className="logview-drop"
            onDrop={this.drop}
          >
            <p>
              Expected to receive 1 file, but received {this.state.files.length}{" "}
              files.
            </p>
          </div>
        );
        ...
        child = (
          ...
          >
            ...
            <p>
              <Button disabled={this.state.processing} onClick={this.upload}>
                Process Log
              </Button>
            </p>
          </div>
      ...
      child = (
        <div className="logview-drop">
          <p>
            If you were using Chrome or Firefox, you could drag and drop a file
            here
          </p>
        </div>
      );
    ...
    return <div className="logview-parent">{child}</div>;
  }
}
