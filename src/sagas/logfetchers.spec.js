  ...
  test("resolves-200", function (done) {
    ...
    return expectSaga(logfetchers, action)
      ...
      .then((result) => {
        ...
        expect(effects.call[2].CALL.args).toEqual([
          "fetchLogkeeper-build-test.json",
        ]);
        expect(effects.call[5].CALL.fn).toBe(writeToCache);
        expect(effects.call[5].CALL.args).toEqual([
          "fetchLogkeeper-build-test.json",
        ]);
...
describe("fetchEvergreen", function () {
  ...
  test("resolves-200-test-by-name", function (done) {
    const resp = new Response(new Blob(["evergreen-test-by-name"]), {
      status: 200,
    });
    const mock = sinon.stub().resolves(resp);
    sinon.replace(evergreenApi, "fetchEvergreen", mock);
    ...
    return expectSaga(logfetchers, action)
      ...
      .then((result) => {
        ...
        expect(effects.call[2].CALL.fn).toBe(readFromCache);
        expect(effects.call[2].CALL.args).toEqual([
          "fetchEvergreen-test-taskid-5-testid.json",
        ]);
        expect(effects.call[5].CALL.fn).toBe(writeToCache);
        expect(effects.call[5].CALL.args).toEqual([
          "fetchEvergreen-test-taskid-5-testid.json",
