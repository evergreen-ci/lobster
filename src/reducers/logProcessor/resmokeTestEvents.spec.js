...
describe("events", function () {
  ...
  test("validate-collections", function () {
    ...
    const rawFilePath =
      path.resolve(".") +
      "/src/reducers/logProcessor/validateCollectionsRaw.txt";
    const rawFile = fs.readFileSync(rawFilePath).toString();
    const state = resmoke(inState, rawFile);
    const validateCollectionsPath = path.resolve(
      "." + "/src/reducers/logProcessor/validateCollections.json"
    );
    const validateCollectionsFile = fs.readFileSync(
      validateCollectionsPath,
      "utf8"
    );
