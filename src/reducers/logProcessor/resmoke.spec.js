...
describe("resmoke", function () {
  test("store-line-gitref", function () {
    ...
    expect(
      "https://github.com/mongodb/mongo/blob/master/src/mongo/shell/shell_utils_launcher.cpp#L447"
    ).toBe(state.lines[2].gitRef);
    expect(state.colorMap[":primary]"]).toBe("#5aae61");
    Object.keys(state.colorMap).forEach((value) =>
